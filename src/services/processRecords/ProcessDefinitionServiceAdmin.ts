...
export class ProcessDefinitionServiceAdmin {
  ...
  static async update(
    ...
    try {
      ...
      if (data.category_id || data.process_code) {
        ...
        // Generar código ISO: category_id-process_code
        if (categoryId && processCode) {
          updateData.codigo = `${categoryId}-${processCode}`;
        }
      }
  ...
  static async publicarNuevaVersion(
    ...
    try {
      ...
      const nuevaVersion = currentDoc.version ? currentDoc.version + 1 : 2;

      const newDocData = {
        codigo: cambios.codigo || currentDoc.codigo,
        ...
        categoria: cambios.categoria || currentDoc.categoria,
        documento_origen_id:
          cambios.documento_origen_id ?? currentDoc.documento_origen_id,
        puesto_responsable_id:
          cambios.puesto_responsable_id ?? currentDoc.puesto_responsable_id,
        jefe_proceso_id: cambios.jefe_proceso_id ?? currentDoc.jefe_proceso_id,
        jefe_proceso_nombre:
          cambios.jefe_proceso_nombre ?? currentDoc.jefe_proceso_nombre,
        ...
        documentos_ids:
          cambios.documentos_ids || currentDoc.documentos_ids || [],
        tipo_registros:
      ...
      console.log(
        `[ProcessDefinitionServiceAdmin] Nueva versión ${nuevaVersion} creada:`,
        newDocRef.id
      );
      return newDocRef.id;
    } catch (error) {
