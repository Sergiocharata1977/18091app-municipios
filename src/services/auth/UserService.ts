...
export class UserService {
  ...
  static async getById(userId: string): Promise<User | null> {
    try {
      ...
      // Usar un organization_id por defecto si no tiene (para compatibilidad)
      const DEFAULT_ORG = 'org_los_senores_del_agro';
      let organizationId = data.organization_id || DEFAULT_ORG;

      if (!data.organization_id && data.rol !== 'super_admin') {
        console.warn(
          `[UserService] User ${userId} has no organization_id, using default: ${DEFAULT_ORG}`
        );
      }
  ...
  static async getByEmail(email: string): Promise<User | null> {
    try {
      ...
      return {
        ...
        rol: data.rol,
        activo: data.activo,
        organization_id: data.organization_id || 'org_los_senores_del_agro',
        modulos_habilitados: data.modulos_habilitados || null,
        created_at: data.created_at?.toDate() || new Date(),
        updated_at: data.updated_at?.toDate() || new Date(),
      } as User;
    ...
    try {
      ...
      return querySnapshot.docs.map(doc => {
        ...
        return {
          ...
          rol: data.rol,
          activo: data.activo,
          organization_id: data.organization_id || 'org_los_senores_del_agro', // Fallback temporal
          modulos_habilitados: data.modulos_habilitados || null,
          created_at: data.created_at?.toDate() || new Date(),
          updated_at: data.updated_at?.toDate() || new Date(),
        } as User;
