...
export class ContextService {
  ...
  static async getContext(
    ...
    // Cargar datos de personnel si existe
    let personnel: PersonnelContextInfo | undefined;
    let assignments: AssignmentsContext | undefined;

    if (user.personnelId) {
      const personnelData = await this.loadPersonnel(db, user.personnelId);
      if (personnelData) {
        personnel = personnelData.info;
        assignments = personnelData.assignments;
      }
    }

    // Cargar compliance si está disponible
    const compliance = await this.loadComplianceData(db, organizationId);
